...
def load_hf_weights(
    ...
    if weights_iterator is not None:
        for hf_key, hf_weight in weights_iterator:
            ...
            # we need to convert them to JAX arrays (jax.Array) here.
            # TODO(amacaskill): check with uLLM team for the best practice of this conversion
            # I tried dlpack and torchax.jax_view first, but both had issues.
            hf_weight_jax = None
            error_logs = []

            # --- START of trying to convert pytorch tensors to jax arrays ---
            # TODO(amacaskill): Choose the correct approach that uLLM team recommends.

            # Strategy 1: dlpack 
            try:
                hf_weight_jax = from_dlpack(to_dlpack(hf_weight))
                logger.info(f"Successfully converted tensor '{hf_key}' using 'dlpack'.")
            except Exception as e:
                error_logs.append(f"'dlpack' conversion failed: {e}")

            # Strategy 2: torchax.interop.jax_view
            if hf_weight_jax is None:
                try:
                    hf_weight_jax = jax_view(hf_weight)
                    logger.info(f"Successfully converted tensor '{hf_key}' using 'jax_view'.")
                except Exception as e:
                    error_logs.append(f"'jax_view' conversion failed: {e}")

            # Strategy 3: Try manual bfloat16 conversion (the robust fallback)
            if hf_weight_jax is None:
                try:
                    if hf_weight.dtype == torch.bfloat16:
                        hf_weight_f32 = hf_weight.to(dtype=torch.float32)
                    else:
                        hf_weight_f32 = hf_weight
                    numpy_array = hf_weight_f32.cpu().numpy()
                    hf_weight_jax = jnp.asarray(numpy_array)
                    if hf_weight.dtype == torch.bfloat16:
                        hf_weight_jax = hf_weight_jax.astype(jnp.bfloat16)
                    logger.info(f"Successfully converted tensor '{hf_key}' using manual bfloat16 conversion.")
                except Exception as e:
                    error_logs.append(f"Manual bfloat16 conversion failed: {e}")
            
            if hf_weight_jax is None:
                error_message = (
                    f"All conversion strategies failed for tensor '{hf_key}'. "
                    f"This indicates a critical interoperability issue. Errors:\n"
